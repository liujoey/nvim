return {
  ...
  -- correctly setup mason lsp / dap extensions
  {
    ...
    opts = function(_, opts)
      vim.list_extend(opts.ensure_installed, { "jdtls", "java-test", "java-debug-adapter", "lemminx" })
    end,
  },
      ...
      setup = {
        jdtls = function(_, opts)
          ...
          for _, jar_pattern in ipairs(jar_patterns) do
            for _, bundle in ipairs(vim.split(vim.fn.glob(jar_pattern, true), "\n")) do
            ...
            callback = function()
              ...
              local jdtls_config = vim.tbl_deep_extend("force", opts, {
                on_attach = function(client, buffer)
                  ...
                  end, { buffer = buffer, desc = "Find Callers" })
                  -- TODO: load this from project settings
                  local overrides = {
                    vmArgs = "-Dtestdestdir=/tmp/sandbox",
                  }
                  require("jdtls").setup_dap({ hotcodereplace = "auto", config_overrides = overrides })
